...
function Drop_list_multi_choice(el, customs = { shadow: false, rounded: true }) {
    ...
    tagColor.textColor = tagColor.textColor || "#2C7A7B";
    tagColor.borderColor = tagColor.borderColor || "#2C7A7B";
    ...
    function init() {
        ...
        const selectedSkills = JSON.parse(element.getAttribute('data-selected-skills') || '[]');
    ...
    function setSelectedSkills(selectedSkills) {
        selectedSkills.forEach(skillId => {
            const option = options.find(o => o.value == skillId);
            if (option) {
                // Đánh dấu tùy chọn là đã chọn
                option.selected = true;
                // Tạo và hiển thị tag cho kỹ năng đã chọn
                createTag(option);
            }
        });
